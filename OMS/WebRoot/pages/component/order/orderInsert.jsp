...
<script type="text/javascript">
	...
	$(function() {
		...
		function getProductData(tds){
			...
			$("#contract_unit_price").val(tds.eq(6).text());
			$("#contract_price").val(tds.eq(5).text());
	...
	function deleteContract(el) {
		...
		countContract_sum_money();//新增countContract_sum_money();
	}
	...
	function openEditProductDialog(num, el) {
		...
		var param = {
			...
			buttons : [ {
				...
				click : function() {
					//
					//新增countContract_sum_money();
					
					editProduct(el);
					$(this).dialog('close');
					//alert("保存");
					countContract_sum_money();
				},
				'class' : 'btn btn-primary btn-middle'
			},
				{
	...
	function alertEorr(){
		if(!submit()){
			jConfirm('请根据提示信息填写数据', '提示对话框', null);
		}
	}
	...
	function submit(){
		...
		$.each(product_list_trs1, function(){
 		...
 		$.each(product_list_trs2, function(){
			...
			//0("mememe"+contract_no);
 		/************************************      new add     *******************************************/
 		/*************************************************************************************************/
 		if(contract_no.length == 0 || contract_no == "请输入合同号"){
 			document.getElementById("msg_contract").innerHTML = "<font color=red>合同号不能为空</font>";
 			return false;
 		}else if (!/^[\da-z]+$/i.test(contract_no)) {
			document.getElementById("msg_contract").innerHTML = "<font color=red>请输入半角英数字</font>";
			return false;
		} else if (contract_no.length > 15) {
			document.getElementById("msg_contract").innerHTML = "<font color=red>最大文字数:15</font>";
			return false;
		} else if (agency_user_nm.length == 0 || agency_user_nm == "选择业务员") {
			document.getElementById("msg_choice").innerHTML = "<font color=red>业务员不能为空</font>";
			return false;
		}else if (agency_cd.length == 0) {
			document.getElementById("msg_choice").innerHTML = "<font color=red>代理商不能为空</font>";
			return false;
		} else if (customer_type.length == 0) {
			document.getElementById("msg_customerType").innerHTML = "<font color=red>客户类别不能为空</font>";
			return false;
		} else if (customer_nm.length == 0 || customer_nm == "请选择") {
			document.getElementById("msg_customerNm").innerHTML = "<font color=red>客户名称不能为空</font>";
			return false;
		} else if (project_nm.length > 70) {
			document.getElementById("msg_project_nm").innerHTML = "<font color=red>最大文字数:70</font>";
			return false;
		} else if (expected_send_month.length == 0 || expected_send_month == "输入发货月") {
			document.getElementById("msg_expected_send_month").innerHTML = "<font color=red>发货月不能为空</font>";
			return false;
		} else if (expected_send_month.match(/^\b[1-3]\d{3}(0[1-9]|1[0-2])$/) == null) {
			document.getElementById("msg_expected_send_month").innerHTML = "<font color=red>请输入YYYYMM格式</font>";
			return false;
		} else if (expected_energize_month.length == 0 || expected_energize_month == "输入送电月") {
			document.getElementById("msg_expected_energize_month").innerHTML = "<font color=red>送电月不能为空</font>";
			return false;
		} else if (expected_energize_month.match(/^\b[1-3]\d{3}(0[1-9]|1[0-2])$/) == null) {
			document.getElementById("msg_expected_energize_month").innerHTML = "<font color=red>请输入YYYYMM格式</font>";
			return false;
		} else if (shelf_months.length == 0 || shelf_months == "输入月数") {
			document.getElementById("msg_shelf_months").innerHTML = "<font color=red>保证月数不能为空</font>";
			return false;
		} else if (!/^[0-9]+$/.test(shelf_months)) {
			document.getElementById("msg_shelf_months").innerHTML = "<font color=red>请输入数字</font>";
			return false;
		} else if (shelf_months.length > 2) {
			document.getElementById("msg_shelf_months").innerHTML = "<font color=red>最大文字数:2</font>";
			return false;// /^\d{1,11}(\.\d{1,2})?$/ 非强制保留小数位的
		} else if (!/^\d{1,11}\.\d{1,2}?$/.test(bid_cu_price)) {
			document.getElementById("msg_bid_cu_price").innerHTML = "<font color=red>铜价的小数点前不能超过11位数，后不能超过2位数</font>";
			return false;
		} else if (!/^\d{1,11}\.\d{1,2}?$/.test(bid_sum_money)) {
			document.getElementById("msg_bid_sum_money").innerHTML = "<font color=red>金额的小数点前不能超过11位数，后不能超过2位数</font>";
			return false;
		} else if (!/^\d{1,2}\.\d{1,2}?$/.test(proportion)) {
			document.getElementById("msg_proportion").innerHTML = "<font color=red>雇佣率的小数点前不能超过2位数，后不能超过2位数</font>";
			return false;
		} else if (!((/^\d{1,2}\.\d{1,2}?$/.test(payments_proportion1))
				&& (/^\d{1,2}\.\d{1,2}?$/.test(payments_proportion2))
				&& (/^\d{1,2}\.\d{1,2}?$/.test(payments_proportion3))
				&& (/^\d{1,2}\.\d{1,2}?$/.test(payments_proportion4))
				&& (/^\d{1,2}\.\d{1,2}?$/.test(payments_proportion5)) && (/^\d{1,2}\.\d{1,2}?$/
				.test(payments_proportion6)))) {
			document.getElementById("payment").innerHTML = "<font color=red>请再所有<u>回款比例</u>中输入数字且小数点前不能超过2位数，后不能超过2位数</font>";
			return false;
		} else if ((expected_payments_date1
				.match(/^\b[1-3]\d{3}(0[1-9]|1[0-2])$/) == null)
				|| (expected_payments_date2
						.match(/^\b[1-3]\d{3}(0[1-9]|1[0-2])$/) == null)
				|| (expected_payments_date3
						.match(/^\b[1-3]\d{3}(0[1-9]|1[0-2])$/) == null)
				|| (expected_payments_date4
						.match(/^\b[1-3]\d{3}(0[1-9]|1[0-2])$/) == null)
				|| (expected_payments_date5
						.match(/^\b[1-3]\d{3}(0[1-9]|1[0-2])$/) == null)
				|| (expected_payments_date6
						.match(/^\b[1-3]\d{3}(0[1-9]|1[0-2])$/) == null)) {
			document.getElementById("payment").innerHTML = "<font color=red>请在所有<u>回款月</u>中以YYYYMM格式输入</font>";
			return false;
		} else if (!((/^\d{1,11}\.\d{1,2}?$/.test(expected_payments_sum1))
				&& (/^\d{1,11}\.\d{1,2}?$/.test(expected_payments_sum2))
				&& (/^\d{1,11}\.\d{1,2}?$/.test(expected_payments_sum3))
				&& (/^\d{1,11}\.\d{1,2}?$/.test(expected_payments_sum4))
				&& (/^\d{1,11}\.\d{1,2}?$/.test(expected_payments_sum5))
				&& (/^\d{1,11}\.\d{1,2}?$/.test(expected_payments_sum6)))) {
			document.getElementById("payment").innerHTML = "<font color=red>请再所有<u>回款金额</u>中输入数字且小数点前不能超过11位数，后不能超过2位数</font>";
			return false;
		}else if (!(product_list.length > 0)) {
			document.getElementById("msg_detail").innerHTML = "<font color=red>请添加订单详细</font>";
			return false;
		}
		/********************************************         new add      ***************************************************************/
		else{
	...
	function clear_msg(){
		document.getElementById("msg_contract").innerHTML = "<font color=red></font>";
		document.getElementById("msg_choice").innerHTML = "<font color=red></font>";
		document.getElementById("msg_customerType").innerHTML = "<font color=red></font>";
		document.getElementById("msg_customerNm").innerHTML = "<font color=red></font>";
		document.getElementById("msg_project_nm").innerHTML = "<font color=red></font>";
		document.getElementById("msg_expected_send_month").innerHTML = "<font color=red></font>";
		document.getElementById("msg_expected_energize_month").innerHTML = "<font color=red></font>";
		document.getElementById("msg_shelf_months").innerHTML = "<font color=red></font>";
		document.getElementById("msg_bid_cu_price").innerHTML = "<font color=red></font>";
		document.getElementById("msg_bid_sum_money").innerHTML = "<font color=red></font>";
		document.getElementById("msg_proportion").innerHTML = "<font color=red></font>";
		document.getElementById("payment").innerHTML = "<font color=red></font>";
	}
	
	function clearMsg_no() {
		document.getElementById("msg_contract").innerHTML = "";
	}
	function clearMsg_choice() {
		document.getElementById("msg_choice").innerHTML = "";
	}
	function clearMsg_type() {
		document.getElementById("msg_customerType").innerHTML = "";
	}
	function clearMsg_customerNm() {
		document.getElementById("msg_customerNm").innerHTML = "";
	}
	function clearMsg_project_nm() {
		document.getElementById("msg_project_nm").innerHTML = "";
	}
	function clearMsg_expected_send_month() {
		document.getElementById("msg_expected_send_month").innerHTML = "";
	}
	function clearMsg_expected_energize_month() {
		document.getElementById("msg_expected_energize_month").innerHTML = "";
	}
	function clearMsg_shelf_months() {
		document.getElementById("msg_shelf_months").innerHTML = "";
	}
	function clearMsg_bid_cu_price() {
		document.getElementById("msg_bid_cu_price").innerHTML = "";
	}
	function clearMsg_bid_sum_money() {
		document.getElementById("msg_bid_sum_money").innerHTML = "";
	}
	function clearMsg_proportion() {
		document.getElementById("msg_proportion").innerHTML = "";
	}
	function clearMsg_payment() {
		document.getElementById("payment").innerHTML = "";
	}
	function clearMsg_detail() {
		document.getElementById("msg_detail").innerHTML = "";
	}
	
	function countCommission() {
		//alert("shit");
		var contract_sum_money = document.forms[0].contract_sum_money.value;
		alert("hahaha:"+contract_sum_money);
		var proportion = document.forms[0].proportion.value;
		var commission = contract_sum_money * proportion * 0.01;
		$('#commission').val(commission.toFixed(2));
		//return;
		
	}
	
	function countContract_sum_money(){
		
		var sum = Number(0);
		for(var num=0;num<2;num++){
			var $trList = $('#product_list_table tbody').eq(num).find("tr");
			for(var i=0;i<$trList.size();i++){
				sum += Number($trList.eq(i).find("td").eq(6).text());				
			}
		}
		$('#contract_sum_money').val(sum.toFixed(2));
		countCommission();
		//return;
	}
</script>
	...
	<div class="main">
										...
										<tbody>
											<tr>
												...
												<td colspan="3">
													<div style="margin-bottom:8px;">
															<input onfocus="clearMsg_no()" class="input-xlarge"
																type="text" name="contract_no" id="contract_no"
																style="margin-bottom:3px;width: 120px;ime-mode: disabled"
																value="${order.contract_no}" placeholder="请输入合同号">&nbsp;
															<div style="display:inline">－</div>
															&nbsp;<input class="input-xlarge" type="text" value="1" id="orders_version" name="orders_version"
																readonly
																style="margin-bottom:3px;width: 30px;;ime-mode: disabled"
																value=""><span id="msg_contract"></span>
													</div>
												</td>
											</tr>
											<tr>
													...
													<table>
														<tr>
															...
															<td><i id="user_search" class="icon icon-search"
																...
																title="查询" onclick="clearMsg_choice()"></i></td>
															
															<td>
															<input type="hidden" id="agency_user_cd" name="agency_user_cd" value="" /> 
															<input type="text" disabled style="width:80px;" id="agency_user_nm" name="agency_user_nm" value="" placeholder="选择业务员"></input>
															</td>
  															<%
																}else{%>
																...
																<td>
															<input type="hidden" id="agency_user_cd" name="agency_user_cd" value="<%=dto.getUser_cd() %>" /> 
															<input type="text" disabled style="width:80px;" id="agency_user_nm" name="agency_user_nm" value="<%=dto.getUser_nm() %>" placeholder="选择业务员"></input>
															</td>	
															<%	}
															%>
															<td>
															...
															<td><i id="agency_search" class="icon icon-search"
																style="background-position: 0 0;padding-top: 0; height: 20px; width: 20px; padding-left: 0;"
																title="查询" onclick="clearMsg_choice()"></i> <i id="agency_clear"
															...
															<td>
																<div
																	...
																	<input type="text" disabled style="width: 40px;"
																		value="" id="agency_cd"></input>
																	<label style="display: inline; margin-left: 3px;"
																		id="agency_nm"></label>
																</div>
																<span id="msg_choice"></span>
															</td>
														</tr>
													</table>
												</td>
											...
											<tr>
												...
												<td colspan="3"><i id="customer_search"
													...
													title="查询" onclick="clearMsg_customerNm()"></i>
													<div style="display: inline; margin-left: 3px;">
														<input type="hidden" id="customer_cd" value=""/>
														<input type="text" id="customer_nm" disabled style="width: 300px;"
															value=""  placeholder="请选择客户名称"/>
														<span id="msg_customerNm"></span>
													</div></td>
											</tr>
											<tr>
												<td class="right_align">项目名称&nbsp;:&nbsp;</td>
												<td colspan="3">
												<input id="project_nm" class="" value="" type="text" onfocus="clearMsg_project_nm()" />
												<span id="msg_project_nm"></span>
												</td>
											</tr>
											<tr>
												<td class="right_align">预计送电月&nbsp;:&nbsp;</td>
												<td colspan="3">
												<input id="expected_energize_month" type="text" value=""  placeholder="选择预计送电月" onfocus="clearMsg_expected_send_month()"
													style="width: 100px; margin-bottom: 3px; ime-mode: disabled"/>
													<img src="../../../images/cal.gif" id="imgs" onfocus="clearMsg_expected_energize_month()"
													onclick="changeMonth('expected_energize_month')&clearMsg_expected_energize_month()" />
													
													<span id="msg_expected_send_month"></span>
												</td>
											</tr>
											<tr>
												<td class="right_align">预计发货月&nbsp;:&nbsp;</td>
												<td colspan="3"><input placeholder="选择预计发货月" style="width: 100px;" id="expected_send_month" type="text"
													value=""/> 
						<img src="../../../images/cal.gif" id="imgs" onclick="changeMonth('expected_send_month')&clearMsg_expected_send_month()" />
											...
											<tr>
												...
												<td colspan="3"><input type="text" id="shelf_months"
													value=""  placeholder="输入月数" 
													onfocus="clearMsg_shelf_months()"
													style="text-align:right;margin-bottom: 0; width: 60px;"></input>
												</td>
											</tr>
											<tr>
												<td></td>
												<td><span id="msg_expected_energize_month"></span></td>
												<td colspan="2"><span id="msg_shelf_months"></span></td>
											</tr>
										</tbody>
									</table>
								</td>
										...
										<tbody>
											<tr>
												...
												<td><input type="text" id="bid_cu_price"
													value="" placeholder="小数点后保留两位" 
													onfocus="clearMsg_bid_cu_price()"
													style="margin-bottom: 5px; width: 150px; text-align: right;"></td>
											</tr>
											<tr>
													<td colspan="2"><span id="msg_bid_cu_price"></span></td>
											</tr>
											<tr>
												<td class="right_align">金额(货币)&nbsp;:&nbsp;</td>
												<td><input type="text" id="bid_sum_money"
													value="" placeholder="请输入数字" onfocus="clearMsg_bid_sum_money()"
													style="margin-bottom: 5px; width: 150px; text-align: right; ime-mode: disabled">
													<select id="currency" name="currency" style="width: 90px;" onclick="clearMsg_choice()">
														<%
											...
											<tr>
												<td colspan="2"><span id="msg_bid_sum_money"></span></td>
											</tr>
											<tr>
												<td class="right_align">合同总价&nbsp;:&nbsp;</td>
												<td><input id="contract_sum_money" class=""
													type="text" value=""
											...
											<tr>
												...
												<td><input type="text" id="proportion"
													value="" placeholder="保留两位" onfocus="clearMsg_proportion()" 
													onblur="countCommission()"
													style="margin-bottom: 5px; width: 50px; text-align: right; ime-mode: disabled">&nbsp;<label
													style="display:inline;">%</label></td>
											</tr>
											<tr>
												<td colspan="2"><span id="msg_proportion"></span></td>
											</tr>
											<tr>
								...
								<div>
									...
									<div
										...
										<table style=" margin-left:30px;width:90%">
											...
											<tr>
												<td colspan="6"><span id="payment"></span></td>
											</tr>
										</table>
									</div>
								</div>
					...
					<ul class="nav nav-tabs" style="width:914px; margin-bottom: 0px;">
						...
						<li class=""><a id="li_2" href="#div_list_1"
							...
							<span id="msg_detail" 
								...
								font-size: 14px; background-color: #FFFFFF;"></span>
					</ul>
				</div>
							...
							<div>
								<div id="" style="float: right;">
									<a id="addProduct1" class="icon icon-add"
										href="javascript:void(0);" onclick="clearMsg_detail()" title="" style="margin-right: 10px">增加</a>
								</div>
		...
		<div class="bottom_block">
			<a class="btn btn-primary btn-middle" style=""
				onclick="clear_msg()&alertEorr()"><!-- submit() -->&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;确认&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
			<a class="btn btn-info btn-middle-aft" href="<%=basePath%>loginAction?method=main">返回</a>
		</div>
	</div>
					...
					<table class="table-edit table-bordered table-striped">
						...
						<tr>
							...
							<td><input type="text" class="must right_align"
								...
								onblur="checkOrderDetailOthers()"  placeholder="请输入合同数量" 
						...
						<tr>
							...
							<td><input type="text" class="must right_align"
								...
								value="" onpropertychange="checkOrderDetailOthers()"
								onblur="checkOrderDetailOthers()"  placeholder="请输入金额" 
								oninput="checkOrderDetailOthers()" /></td>
						</tr>
						<tr>
